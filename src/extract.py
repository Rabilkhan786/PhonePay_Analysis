...
def extract(raw):
    ...
    for family in ["transaction", "user", "merchant"]:
        ...
        for path in sorted(base.rglob("*.json")):
            ...
            if len(rel) == 2:
                state, level = None, "state"
            elif len(rel) == 4 and rel[0] == "state":
                state, level = clean_name(rel[1]), "district"
            else:
            ...
            items = (
                ...
                else [{"name": k, **v} for k, v in obj["hoverData"].items()]
            )
            for item in items:
                ...
                row = dict(
                    level=level,
                    state=state or name,
                    district=name if state else "",
                    year=year,
                    quarter=quarter,
                    period_id=year * 4 + quarter - 1,
                    family=family,
                    raw_name=item["name"],
                    source_path=manifest[-1]["path"],
                )
                if family == "transaction":
                    metrics = [m for m in item["metric"] if m["type"] == "TOTAL"]
                    assert len(metrics) == 1
                    row.update(transactions=metrics[0]["count"], value_inr=metrics[0]["amount"])
                else:
                    row["registered_" + ("users" if family == "user" else "merchants")] = item[
                        "registeredCount"
                    ]
                records.append(row)
    ...
    for path in sorted(base.rglob("*.json")):
        ...
        for item in json.loads(payload)["data"]["transactionData"]:
            m = [m for m in item["paymentInstruments"] if m["type"] == "TOTAL"]
            assert len(m) == 1
            categories.append(
                dict(
                    state=state,
                    ...
                    quarter=quarter,
                    period_id=year * 4 + quarter - 1,
                    category_raw=item["name"],
                    transactions=m[0]["count"],
                )
            )
    cat = pd.DataFrame(categories)
    assert not cat.duplicated(["state", "period_id", "category_raw"]).any()
    cat.to_csv(out / "category_quarter.csv", index=False)
    long[["level", "state", "district", "family", "raw_name", "source_path", "period_id"]].to_csv(
        out / "source_lineage.csv", index=False
    )
    ...
    print(
        "Nonzero geographic differences:",
        (reconciliation.filter(like="_difference").abs() > 0.01).sum().to_dict(),
    )
